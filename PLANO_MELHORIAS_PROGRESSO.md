...
## Rodada 2026-09-11 (2ª) — performance de render (v14)

Relato: FPS baixo. Diagnóstico em browser (headless, main canvas 1600×900 a DPR 1.25): `Debug.frameTime` (update+draw JS) ficava em **1.4–3.4 ms** — o JS **não** era o gargalo; o custo estava na **rasterização/composição do canvas**. O caminho de render fazia **4 operações full-canvas por frame**: `clearRect` + blit do mundo (`_drawWorldLayer`) + blit do overlay de luz (`_drawLightingLayer`, canvas em resolução cheia) + fill de um gradiente radial de vignette 1600×900 (`drawPostProcess`). Como luz e vignette rasterizavam **sempre em resolução cheia**, o `_autoResScale` não os barateava: o auto-ajuste encolhia só o mundo e ficava **preso no mínimo 0.6** sem nunca recuperar o FPS (medido no código antigo: **34 fps** de entrega, `_autoResScale` 0.6 fixo em todas as janelas de amostragem).

**Fix (v14):** luz e vignette estático passam a ser **compostados no buffer do mundo** (baixa resolução, segue `_autoResScale`):
1. `_drawLightingLayer` escreve todas as operações de luz (blit cacheado, tint `overlay`, "air" `lighter`, raios do player) em `wctx` via `const T = (gameState === 'WORLD_MAP') ? ctx : wctx` (o path do mapa segue em `ctx` — o mapa desenha direto no canvas principal; estado `WORLD_MAP` é hoje inalcançável, código mantido guardado).
2. O blit do mundo saiu de `_drawWorldLayer`; um novo `Debug.time('composite', …)` em `draw()` desenha o vignette estático no `wctx` e faz **um único blit** para o principal — **2 operações full-canvas por frame** (era 4).
3. `ScreenEffects.drawPostProcess` mantém no canvas principal apenas os vignettes **dinâmicos** (boss-death `_vignetteAlpha`, pulso vermelho de HP < 30%); o vignette constante agora está baked no buffer (mesmo gradiente/alpha).

**Ganho estrutural:** luz e vignette agora **escalam com o auto-res** — o auto-ajuste passa a cobrir o render inteiro, o que resolve o caso de máquina fraca (antes o custo fixo full-canvas ficava fora do seu alcance).

**Verificação (browser real, harness CDP main-world):** A/B no mesmo disco (código velho via `git stash` vs novo) — velho: 272/276 rajadas de rAF por 8s (**~34 fps**), `_autoResScale` 0.6 constante; novo: auto-res **recupera em janela longa** (0.6→0.67→0.74→0.82) ao invés de travar. `errors: 0` em janelas de 6s, 8s e 30s (rodadas limpas, sem instrumentação) e no boot v14. Pulso low-HP intacto (canal R oscila +8 acima de G no pico; R<B no vale). Paridade do vignette verificada por pixel: o gradiente faz exatamente o mesmo ~7.5% de escurecimento no canto que a versão anterior (radius 1.2× a diagonal → o canto nunca atinge os 0.13 do stop final, nos dois casos); gradiente aplicado ao `wctx` confirmado pixel a pixel. Smoke de painéis (loja/skills abrir-fechar, 0 erros). `check-js` 54/54, `node --check sw.js` OK.

**Nota de harness:** `Debug.time` NÃO deve ser monkey-patchado entre células de eval — o restore por propriedade (`Debug.time = Debug.time.__orig`) quebra se a propriedade for salva no objeto original em vez do wrapper, deixando `Debug.time = undefined` e enchendo `Debug.errors` de "Debug.time is not a function" (erro de instrumentação, não do jogo). Medições de entrega de rAF no headless oscilam muito (30–215 fps na mesma build, sob carga do sync do Drive) — a régua confiável é a trajetória do `_autoResScale`, não o contador de frames bruto.

`CACHE_VERSION` v13→v14 + `UPDATE_LOG.current` + changelog pt/en/es. Commit `9650583`; GitHub Pages já servindo `krevathorn-v14`. Screenshots em `.omp/perf-novo-{gameplay,lowhp,vig}.png`.
